...
foo = numpy.fromfile('in_matX.np')
sxsolve_x[:] = foo.reshape(sizeX,round(foo.size/sizeX))[:,-1]
foo = numpy.fromfile('in_matP.np')
sxsolve_step[:] = foo.reshape(sizeX,round(foo.size/sizeX))[:,-1]
foo = []
...
for epoch in range(numEpochs):  # loop over the dataset multiple times
    ...
    matX[:,epoch+1] = sxsolve_x[:]
    matP[:,epoch+1] = sxsolve_step[:]
    rvcF[epoch] = running_loss
