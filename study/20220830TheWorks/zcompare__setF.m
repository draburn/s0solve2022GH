...
case 3001
	...
	matJ0 += 1.0E-8*randn(sizeF,sizeX);
	%
	matJE = matSF*matJ0/matSX;
	matA0 = 1.0E-8*matSF*randn(sizeF,sizeX)/matSX;
	matA1 = randn(sizeX,sizeX)/matSX;
...
case 3002
	...
	%
	matJE = matSF*matJ0/matSX;
	matA0 = 1.0E-7*matSF*randn(sizeF,sizeX)/matSX;
	matA1 = randn(sizeX,sizeX)/matSX;
	matA2 = randn(sizeX,sizeX)/matSX;
	matB0 = 1.0E-7*matSF*randn(sizeF,sizeX)/matSX;
	matB1 = randn(sizeX,sizeX)/matSX;
	matB2 = randn(sizeX,sizeX)/matSX;
	matB3 = randn(sizeX,sizeX)/matSX;
	y = @(x)( x - vecXE );
...
case 3009
	...
	matJ0 += 1.0E-2*randn(sizeF,sizeX);
	%
	matJE = matSF*matJ0/matSX;
	matA0 = 0.0E0*matSF*randn(sizeF,sizeX)/matSX;
	matA1 = randn(sizeX,sizeX)/matSX;
...
case 12000
	...
	sizeF = sizeX;
	%
	rawdat = load( "dat/dat_82445x31_202208.m" );
	matJ0 = rawdat.matJ;
	vecX1 = rawdat.vecX0;
	vecF1 = rawdat.vecF0;
	%
	vecXE = vecX1 - (matJ0\vecF1);
	y = @(x)( x - vecXE );
	funchF = @(x)( matJ0*y(x) );
	matJ = matJ0;
	%
	vecX0 = vecXE + (randn(sizeX,1).*(vecX1-vecXE));
otherwise
	error( "Invalid fType." );
endswitch
