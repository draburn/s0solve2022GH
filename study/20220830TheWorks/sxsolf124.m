%sxsolf124 has a crude attempted restart with upscaling epsilon FD. It doesn't help.
...
function [ vecX, vecF, retCode, fevalCount, stepsCount, datOut ] = sxsolf100( funchF, vecX_initial, vecF_initial=[], prmIn=[] )
	...
	while (1)
		...
		if ( 0~= retCode )
			...
			msg( __FILE__, __LINE__, "*                                                                         *" );
			msg( __FILE__, __LINE__, "*                      KAABOOOOOOOOOOOOOOOOOOOOOOOOM!!!!                  *" );
			msg( __FILE__, __LINE__, "*                                                                         *" );
			msg( __FILE__, __LINE__, "***************************************************************************" );
			
			
			prm.epsFD = norm(vecX_initial-vecX);
			fModelDat = [];
			vecX_next = [];
			vecF_next = [];
			vecX = vecX_initial;
			vecF = vecF_initial;
			omega = sumsq(vecF_initial)/2.0;
			continue;
		endif
