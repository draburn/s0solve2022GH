...
if (numPts > 0):
	...
	for n in range(numPts):
		...
		temp_f, temp_fVar = pytorchCIFAR10demo.eval_epoch_loss(vecX0+temp_vecD)
...
vecFG2KLo = smooth( rvcFG2[0,:], 2 )
vecFG2KHi = smooth( rvcFG2[0,:], 20 )

msg(f'Generating plot...')

plt.plot(np.sqrt(np.sum(matDG2*matDG2,0)),rvcFG2[0,:],'x')
plt.plot(np.sqrt(np.sum(matDG2*matDG2,0)),vecFG2KLo,'.-')
plt.plot(np.sqrt(np.sum(matDG2*matDG2,0)),vecFG2KHi,'o-')
if (numPts > 0):
