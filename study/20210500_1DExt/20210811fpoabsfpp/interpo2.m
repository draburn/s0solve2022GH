...
xVals = [ xVals; 0.031 ]; numPts++;
xVals = [ xVals; 0.0166 ]; numPts++;
xVals = [ xVals; 0.002 ]; numPts++;
xVals = [ xVals; -0.006 ]; numPts++;
...
numFigs++; figure(numFigs);
plot( ...
  cent(modXVals), diff(cap(modHVals,-hCap,hCap))./diff(modXVals), 'x-', 'linewidth', 2, ...
  xVals, 0*xVals, 'ko-' );
grid on;