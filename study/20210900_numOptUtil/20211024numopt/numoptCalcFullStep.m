function [ vecDeltaFinal, retCode, datOut ] = numoptCalcFullStep( omega0, vecG, matH, prm=[] )
	...
	iota1 = omega0 - omega1;
	vecDeltaP1 = -(matR1 \ (matR1'\vecDelta1) );
	iotaP1 = -( ( vecG + (matH*vecDelta1))' * vecDeltaP1 );
	iotaTarget = 1.2*(omega0 - omegaThresh); % Coeff is free param.
	mu2 = mu1 * ( (iotaTarget/iota1)^(iota1/(iotaP1*mu1)) );
	if (isrealscalar(deltaNormThresh))
		kappa1 = vecDelta1' * vecDelta1;
		kappaP1 = 2.0 * (vecDelta1' * vecDeltaP1);
		kappaThresh = deltaNormThresh^2;
		kappaTarget = 1.2 * kappaThresh; % Coeff is free param.
		mu2Kappa = mu1 * ( (kappaTarget/kappa1)^(kappa1/(kappaP1*mu1)) );
		mu2 = max([ mu2, mu2Kappa ]); % Larger mu = smaller delta.
	end
	% Note: we could check to see if mu2 > muExtrap, which we've already tried.
	% But, it almost certainly is, so, for cleanlyness, let's not bother.
