function ary3NablaSqF = ufoCalcNablaSqF( funchF, vecX0, prm=[] )
	%
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	% COMMON INIT.
	%
	commondefs;
	thisFile = "ufoCalcNablaSqF";
	%
	...
	% DRaburn 2021.10.03:
	%  We could allow a different epsFD for the diagonal and off-diagonal elements,
	%  but, KISS.
	%
	%
	%
	...
	% DRaburn 2021.10.03:
	%  Note that if 1 == sizeX, size( ary3NablaSqF ) will be [ sizeF, 1 ],
	%  not [ sizeF, 1, 1 ].
	%
	for n=1:sizeX
		epsFD = vecEpsFD(n);
...
%!test
%!	commondefs;
%!	thisFile = "ufoCalcNablaSqF test 1";
%!	sizeX = 3;
%!	sizeF = sizeX;
%!	funchF = @(x)( (1.0-x).^4 );
%!	vecX0 = zeros(sizeX,1);
...
%!	prm = [];
%!	tic();
%!	ary3NablaSqF = ufoCalcNablaSqF( funchF, vecX0, prm )
%!	toc();
...
%!test
%!	commondefs;
%!	thisFile = "ufoCalcNablaSqF test 2";
%!	setprngstates(0);
%!	sizeX = 3
%!	sizeF = 1
