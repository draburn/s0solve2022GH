...
vecLamdba0 = diag(matLambda0);
[ lambda0Crit, n0Crit ] = min(vecLamdba0);
vecPsi0Crit = matPsi0(:,n0Crit);
matH = matH0 - ( lambda0Crit + muCritApplied )*(vecPsi0Crit*vecPsi0Crit');
%
[ matPsi, matLambda ] = eig(matH);
vecLambda = diag(matLambda);
muCrit = muCritApplied
gammaCrit = vecPsi0Crit'*vecG
hFrob = sqrt(sum(sum(matH.^2)));
gSq = sum(vecG.^2);
%
%
%
...
numMuVals = 100;
muLo = 10.0^floor(log10(muCritApplied)-0.5);
%muLo = muCritApplied*(1.1);
muHi = 10.0^ceil(log10(hFrob)+1.5);
muVals = muLo * ( (muHi/muLo) .^ linspace(0.0,1.0,numMuVals) );
vecDeltaVals = zeros(sizeX,numMuVals);
etaVals = zeros(1,numMuVals);
kappaVals = zeros(1,numMuVals);
for n=numMuVals:-1:1
	mu = muVals(n);
	[ matR, cholFlag ] = chol( matH + mu*matI );
	if (~cholFlag)
		vecDelta = -( matR \ (matR'\vecG) );
		eta = -( vecDelta'*vecG + 0.5*vecDelta'*matH*vecDelta );
		kappa = vecDelta'*vecDelta;
		%
		vecDeltaVals(:,n) = vecDelta;
		etaVals(:,n) = eta;
		kappaVals(:,n) = kappa;
		%
		clear kappa;
...
etaVizVals = etaVals;
kappaVizVals = kappaVals;
%etaVizVals(etaVals>1e7*etaVals(end)) = 0.0;
%kappaVizVals(kappaVals>1e14*kappaVals(end)) = 0.0;
numMuVals2 = 1000;
muVals2 = muLo * ( (muHi/muLo) .^ linspace(0.0,1.0,numMuVals2) );
%
%
%
numFigs++; figure(numFigs);
loglog( ...
  hFrob, gSq/hFrob, 'o', 'markersize', 25, 'linewidth', 2, ...
  hFrob, gSq/hFrob, '+', 'markersize', 25, 'linewidth', 2, ...
  muVals, etaVizVals, 'o-', ...
  muVals2, gSq./muVals2, '-', 'linewidth', 2, ...
  muVals2, gammaCrit^2./(muVals2-muCrit)+muCrit*gammaCrit^2./(muVals2-muCrit).^2, '-', 'linewidth', 2 );
grid on;
%
numFigs++; figure(numFigs);
loglog( ...
  hFrob, gSq/(hFrob^2), 'o', 'markersize', 25, 'linewidth', 2, ...
  hFrob, gSq/(hFrob^2), '+', 'markersize', 25, 'linewidth', 2, ...
  muVals, kappaVizVals, 'o-', ...
  muVals2, gSq./(muVals2.^2), '-', 'linewidth', 2, ...
  muVals2, gammaCrit^2./(muVals2-muCrit).^2, '-', 'linewidth', 2 );
grid on;
%
numFigs++; figure(numFigs);
loglog( ...
  muVals, etaVals.*muVals/gSq, 'o-', ...
  muVals, kappaVals.*(muVals.^2)/gSq, 'o-' );
grid on;

return;


muCritApplied = exp(randn());
muCritGuess = 0.0;
%muCritGuess = 4.7971
omega0 = exp(randn());
...
%
matA0 = randn(sizeF,sizeX).*exp(randn(sizeF,sizeX));
matH0 = matA0'*matA0;
...
vecG = randn(sizeX,1).*exp(randn(sizeX,1));
return
%
matI = eye(sizeX,sizeX);
sizeMu = 1000;
