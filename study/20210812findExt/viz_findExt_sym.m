function viz_findExt_sym( xVals, fVals, wVals=[], xExt=[], p=[], prm=[] )
	%
	commondefs;
	thisFile = "viz_findExt_sym";
	...
	%
	%
	xExtMin = mygetfield( prm, "xExtMin", min([ xExt,  min(xVals) ]) );
	xExtMax = mygetfield( prm, "xExtMax", max([ xExt,  max(xVals) ]) );
	pMin = mygetfield( prm, "pMin", min([ p, 0.1 ]) );
	pMax = mygetfield( prm, "pMax", max([ p, 9.1 ]) );
	xExtSize = mygetfield( prm, "xExtSize", 200 );
	pSize = mygetfield( prm, "pSize", 201 );
	numContours = mygetfield( prm, "numContours", 20 );
	numColors = mygetfield( prm, "numColors", 21 );
	if ( valLev >= VALLEV__LOW )
		assert( isrealscalar(xExtMin) );
		assert( isrealscalar(xExtMax) );
		assert( isrealscalar(pMin) );
		assert( isrealscalar(pMax) );
		assert( isposintscalar(xExtSize) );
		assert( isposintscalar(pSize) );
		assert( xExtMin < xExtMax );
		assert( pMin < pMax );
	...
	%
	%
	[ omega, fExt, f1 ] = findExt_sym__calcResMesh( xExt, p, xVals, fVals, wVals );
	xExtVals_forMesh = linspace( xExtMin, xExtMax, xExtSize );
	pVals_forMesh = linspace( pMin, pMax, pSize );
	[ xExtMesh, pMesh ] = meshgrid( xExtVals_forMesh, pVals_forMesh );
	[ omegaMesh, fExtMesh, f1Mesh ] = findExt_sym__calcResMesh( xExtMesh, pMesh, xVals, fVals, wVals );
	size1 = size(xExtVals_forMesh,1);
	size2 = size(xExtVals_forMesh,2);
	%
	zMesh = sqrt(omegaMesh);
	zMesh = cap( zMesh, 0.0, ...
	  (numContours+1.0)/(numContours-1.0)*max([ median(reshape(zMesh,1,[])), sqrt(omega) ]) );
	numFigs++; figure(numFigs);
	contourf( xExtVals_forMesh, pVals_forMesh, zMesh, numContours );
	axis("square");
	%axis("equal");
	...
	hold on;
	plot( ...
	  xExt, p, 'wo', 'linewidth', 2, 'markersize', 20 );
	hold off;
	grid on;
	xlabel( "xExt" );
	ylabel( "p" );
	title( "sqrt(omega) vs xExt, p" );
	%
	zMesh = fExtMesh;
	%zMesh = cap( zMesh, 0.0, ...
	%  (numContours+1.0)/(numContours-1.0)*max([ median(reshape(zMesh,1,[])), fExt ]) );
	numFigs++; figure(numFigs);
	contourf( xExtVals_forMesh, pVals_forMesh, zMesh, numContours );
	axis("square");
	%axis("equal");
