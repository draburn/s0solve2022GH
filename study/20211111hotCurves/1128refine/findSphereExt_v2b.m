%function vecX = findSphereExt_v2b( funchGOmega, vecXC, bigR, vecX0, prm=[] )
	thisFile = "findSphereExt_v2b";
	%
	msg( thisFile, __LINE__, "WIP!" );
	msg( thisFile, __LINE__, "matW, ergo ||vecGamma|| min, can go uphill!" );
	%
	sizeX = size(vecXC,1);
	...
	while (1)
		...
		for k=1:sizeX
			...
			epsFD = 0.01;
			vecW = ( funchGamma( vecX + epsFD*vecV ) - funchGamma( vecX - epsFD*vecV ) ) / ( 2.0*epsFD*norm(vecV) );
			matW(:,k) = vecW;
		end
		sizeK = sizeX;
		matWTW = matW'*matW
		vecWTGamma = matW'*vecGamma % Should always be norm(vecGamma)*vecE1.
		...
		while (1)
			...
			echo__mu = mu
			%
			matM = matWTW + (mu*matIK);
			[ matR, cholFlag ] = chol(matM);
			if ( 0 == cholFlag )
				vecDeltaPre_trial = -(matV*(matR\(matR'\vecWTGamma)))
