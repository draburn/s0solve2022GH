%function vecX = findSphereExt_v3( funchGOmega, vecXC, bigR, vecX0, prm=[] )
	thisFile = "findSphereExt_v3";
	%
	msg( thisFile, __LINE__, "WIP!" );
	msg( thisFile, __LINE__, "Is matH symmetric?" );
	%
	sizeX = size(vecXC,1);
	...
	%
	%
	funchGamma = @(vecXDummy)( findSphereExt__calcGamma3( funchG, vecXC, bigR, vecXDummy ) );
	vecX = vecX0;
	%
	...
	while (1)
		...
		sizeK = sizeX;
		matVTHV = matV'*matHV
		% The calculated matVTHV is expected to be symmetric modulo finite differencing.
		% Make fully symmetric.
		matVTHV = ( matVTHV' + matVTHV ) / 2.0;
