function [ matX, matX_everyGeval ] = calcGradDescentCurve_alpha( funchGOmega, vecX0, prm=[] )
	...
	%
	matX(:,1) = vecX0;
	[ vecG0, omega0 ] = funchGOmega( vecX0 ); gevalCount++; matX_everyGeval(:,gevalCount) = vecX0;
	fevalCount = 1;
	...
	while (1) % main loop
		...
		while (1) % BT search.
			...
			%
			vecGD1 = -vecG;
			vecGD2 = -funchGOmega( vecX + 0.5*deltaT*vecGD1 ); gevalCount++; matX_everyGeval(:,gevalCount) = vecX + 0.5*deltaT*vecGD1;
			vecGD3 = -funchGOmega( vecX + 0.5*deltaT*vecGD2 ); gevalCount++; matX_everyGeval(:,gevalCount) = vecX + 0.5*deltaT*vecGD2;
			vecGD4 = -funchGOmega( vecX + deltaT*vecGD3 ); gevalCount++; matX_everyGeval(:,gevalCount) = vecX + deltaT*vecGD3;
			fevalCount += 3;
			...
			[ vecG_trial, omega_trial ] = funchGOmega( vecX_trial ); gevalCount++; matX_everyGeval(:,gevalCount) = vecX_trial;
			fevalCount++;
