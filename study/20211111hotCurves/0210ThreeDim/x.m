...
%
%
if ( doGrad_cnstH_patch )
	numCurves++;
	curveDat(numCurves).startTime = time();
	curveDat(numCurves).strName = 'calcGradCurve_cnstH patch';
	curveDat(numCurves).prm = [];
	curveDat(numCurves).vecXVals = calcGradCurve_cnstH( vecX0, omega0, vecG0, matH0_patch, curveDat(numCurves).prm );
	curveDat(numCurves).numPts = size( curveDat(numCurves).vecXVals, 2 );
	curveDat(numCurves).elapsedTime = time() - curveDat(numCurves).startTime;
	msg( __FILE__, __LINE__, sprintf( "  %s: %d pts in %0.3fs.", ...
	  curveDat(numCurves).strName, curveDat(numCurves).numPts, curveDat(numCurves).elapsedTime ) );
end
if ( doLev_cnstH_patch )
	numCurves++;
	curveDat(numCurves).startTime = time();
...
if ( doGradSeg_cnstH_patch )
	numCurves++;
	curveDat(numCurves).startTime = time();
	curveDat(numCurves).strName = 'calcGradSeg_cnstH patch';
	curveDat(numCurves).prm = [];
	curveDat(numCurves).vecXVals = calcGradSeg_cnstH( vecX0, omega0, vecG0, matH0_patch, curveDat(numCurves).prm );
	curveDat(numCurves).numPts = size( curveDat(numCurves).vecXVals, 2 );
	curveDat(numCurves).elapsedTime = time() - curveDat(numCurves).startTime;
	msg( __FILE__, __LINE__, sprintf( "  %s: %d pts in %0.3fs.", ...
	  curveDat(numCurves).strName, curveDat(numCurves).numPts, curveDat(numCurves).elapsedTime ) );
end
if ( doGradSeg_cnstH_patch_scaled )
	numCurves++;
	curveDat(numCurves).startTime = time();
	curveDat(numCurves).strName = 'calcGradSeg_cnstH patch SCALED';
	curveDat(numCurves).prm = [];
	curveDat(numCurves).prm.matS = diag(diag(matH0_patch));
	curveDat(numCurves).vecXVals = calcGradSeg_cnstH( vecX0, omega0, vecG0, matH0_patch, curveDat(numCurves).prm );
	curveDat(numCurves).numPts = size( curveDat(numCurves).vecXVals, 2 );
	curveDat(numCurves).elapsedTime = time() - curveDat(numCurves).startTime;
	msg( __FILE__, __LINE__, sprintf( "  %s: %d pts in %0.3fs.", ...
