...
def get_dat_from_x(this_x, this_index0, this_index1, this_dat ):
    ...
    ###return torch.Tensor( numpy.reshape( numpy.asarray(this_x[this_index0:this_index1], dtype=numpy.float32), this_dat.size() ) )
    return torch.from_numpy( numpy.reshape( numpy.asarray(this_x[this_index0:this_index1], dtype=numpy.float32), this_dat.size() ) )


size_list = get_size_list(net)
print(f"type(size_list) = {type(size_list)}")
print(f"len(size_list) = {len(size_list)}")
print(f"size_list = {size_list}")
index_list = get_index_list(net)
print(f"type(index_list) = {type(index_list)}")
print(f"len(index_list) = {len(index_list)}")
print(f"index_list = {index_list}")
sxsolve_x = get_x_from_net(net)
print(f"type(sxsolve_x) = {type(sxsolve_x)}")
print(f"len(sxsolve_x) = {len(sxsolve_x)}")
print(f"sxsolve_x[0] = {sxsolve_x[0]:.17e}")
print(f"sxsolve_x[-1] = {sxsolve_x[-1]:.17e}")


# Intermediate state test
# No effect.
#print(f"net.conv1.bias.data[0] = {net.conv1.bias.data[0]:.17e}")
#print("Setting sxsolve_x[0] = 9.0...")
#sxsolve_x[0] = 9.0
#print(f"net.conv1.bias.data[0] = {net.conv1.bias.data[0]:.17e}")
#exit()
...
print("")
print("sxsolve_conv1_bias...")
n = 0
sxsolve_conv1_bias = numpy.reshape(sxsolve_x[index_list[n]:index_list[n+1]],size_list[n])
print(f"type(sxsolve_conv1_bias) = {type(sxsolve_conv1_bias)}")
print(f"sxsolve_conv1_bias.shape = {sxsolve_conv1_bias.shape}")
print(f"sxsolve_conv1_bias[0] = {sxsolve_conv1_bias[0]:.17e}")
print(f"sxsolve_conv1_bias[-1] = {sxsolve_conv1_bias[-1]:.17e}")
net.conv1.bias.data = torch.from_numpy(sxsolve_conv1_bias)
print(f"type(net.conv1.bias.data) = {type(net.conv1.bias.data)}")
print(f"net.conv1.bias.data.size() = {net.conv1.bias.data.size()}")
print(f"net.conv1.bias.data[0] = {net.conv1.bias.data[0]:.17e}")
print(f"net.conv1.bias.data[-1] = {net.conv1.bias.data[-1]:.17e}")

print("Setting sxsolve_x[0] = 0.0...")
sxsolve_x[0] = 0.0
print(f"sxsolve_x[0] = {sxsolve_x[0]:.17e}")
print(f"sxsolve_conv1_bias[0] = {sxsolve_conv1_bias[0]:.17e}")
print(f"net.conv1.bias.data[0] = {net.conv1.bias.data[0]:.17e}")
print("Setting sxsolve_conv1_bias[0] = 1.0...")
sxsolve_conv1_bias[0] = 1.0
print(f"sxsolve_x[0] = {sxsolve_x[0]:.17e}")
print(f"sxsolve_conv1_bias[0] = {sxsolve_conv1_bias[0]:.17e}")
print(f"net.conv1.bias.data[0] = {net.conv1.bias.data[0]:.17e}")
print("Setting net.conv1.bias.data[0] = 2.0...")
net.conv1.bias.data[0] = 2.0
print(f"sxsolve_x[0] = {sxsolve_x[0]:.17e}")
print(f"sxsolve_conv1_bias[0] = {sxsolve_conv1_bias[0]:.17e}")
print(f"net.conv1.bias.data[0] = {net.conv1.bias.data[0]:.17e}")

print("")
print("sxsolve_conv1_weight...")
n = 1
sxsolve_conv1_weight = numpy.reshape(sxsolve_x[index_list[n]:index_list[n+1]],size_list[n])
print(f"type(sxsolve_conv1_weight) = {type(sxsolve_conv1_weight)}")
print(f"sxsolve_conv1_weight.shape = {sxsolve_conv1_weight.shape}")
print(f"sxsolve_conv1_weight[0][0][0][0] = {sxsolve_conv1_weight[0][0][0][0]:.17e}")
print(f"sxsolve_conv1_weight[-1][-1][-1][-1] = {sxsolve_conv1_weight[-1][-1][-1][-1]:.17e}")
net.conv1.weight.data = torch.from_numpy(sxsolve_conv1_weight)
print(f"type(net.conv1.weight.data) = {type(net.conv1.weight.data)}")
print(f"net.conv1.weight.data.size() = {net.conv1.weight.data.size()}")
print(f"net.conv1.weight.data[0][0][0][0] = {net.conv1.weight.data[0][0][0][0]:.17e}")
print(f"net.conv1.weight.data[-1][-1][-1][-1] = {net.conv1.weight.data[-1][-1][-1][-1]:.17e}")

indx = index_list[n]
print("Setting sxsolve_x[{indx}] = 0.0...")
sxsolve_x[indx] = 0.0
print(f"sxsolve_x[{indx}] = {sxsolve_x[indx]:.17e}")
print(f"sxsolve_conv1_weight[0][0][0][0] = {sxsolve_conv1_weight[0][0][0][0]:.17e}")
print(f"net.conv1.weight.data[0][0][0][0] = {net.conv1.weight.data[0][0][0][0]:.17e}")
print("Setting sxsolve_conv1_weight[0][0][0][0] = 1.0...")
sxsolve_conv1_weight[0][0][0][0] = 1.0
print(f"sxsolve_x[{indx}] = {sxsolve_x[indx]:.17e}")
print(f"sxsolve_conv1_weight[0][0][0][0] = {sxsolve_conv1_weight[0][0][0][0]:.17e}")
print(f"net.conv1.weight.data[0][0][0][0] = {net.conv1.weight.data[0][0][0][0]:.17e}")
print("Setting net.conv1.weight.data[0][0][0][0] = 2.0...")
net.conv1.weight.data[0][0][0][0] = 2.0
print(f"sxsolve_x[{indx}] = {sxsolve_x[indx]:.17e}")
print(f"sxsolve_conv1_weight[0][0][0][0] = {sxsolve_conv1_weight[0][0][0][0]:.17e}")
print(f"net.conv1.weight.data[0][0][0][0] = {net.conv1.weight.data[0][0][0][0]:.17e}")

indx = index_list[n+1]-1
print("Setting sxsolve_x[{indx}] = 0.0...")
sxsolve_x[indx] = 0.0
print(f"sxsolve_x[{indx}] = {sxsolve_x[indx]:.17e}")
print(f"sxsolve_conv1_weight[-1][-1][-1][-1] = {sxsolve_conv1_weight[-1][-1][-1][-1]:.17e}")
print(f"net.conv1.weight.data[-1][-1][-1][-1] = {net.conv1.weight.data[-1][-1][-1][-1]:.17e}")
print("Setting sxsolve_conv1_weight[-1][-1][-1][-1] = 1.0...")
sxsolve_conv1_weight[-1][-1][-1][-1] = 1.0
print(f"sxsolve_x[{indx}] = {sxsolve_x[indx]:.17e}")
print(f"sxsolve_conv1_weight[-1][-1][-1][-1] = {sxsolve_conv1_weight[-1][-1][-1][-1]:.17e}")
print(f"net.conv1.weight.data[-1][-1][-1][-1] = {net.conv1.weight.data[-1][-1][-1][-1]:.17e}")
print("Setting net.conv1.weight.data[-1][-1][-1][-1] = 2.0...")
net.conv1.weight.data[-1][-1][-1][-1] = 2.0
print(f"sxsolve_x[{indx}] = {sxsolve_x[indx]:.17e}")
print(f"sxsolve_conv1_weight[-1][-1][-1][-1] = {sxsolve_conv1_weight[-1][-1][-1][-1]:.17e}")
print(f"net.conv1.weight.data[-1][-1][-1][-1] = {net.conv1.weight.data[-1][-1][-1][-1]:.17e}")
