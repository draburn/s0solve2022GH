...
def get_grad_from_net(dummy_net):
    g = []
    g = get_appended_vec_from_data(g, dummy_net.conv1.bias.grad)
    g = get_appended_vec_from_data(g, dummy_net.conv1.weight.grad)
    g = get_appended_vec_from_data(g, dummy_net.conv2.bias.grad)
    g = get_appended_vec_from_data(g, dummy_net.conv2.weight.grad)
    g = get_appended_vec_from_data(g, dummy_net.fc1.bias.grad)
    g = get_appended_vec_from_data(g, dummy_net.fc1.weight.grad)
    g = get_appended_vec_from_data(g, dummy_net.fc2.bias.grad)
    g = get_appended_vec_from_data(g, dummy_net.fc2.weight.grad)
    g = get_appended_vec_from_data(g, dummy_net.fc3.bias.grad)
    g = get_appended_vec_from_data(g, dummy_net.fc3.weight.grad)
    return g
...
for epoch in range(2):  # loop over the dataset multiple times
    ...
    for i, data in enumerate(trainloader, 0):
        ...
        #exit()
        sxsolve_f = loss
        sxsolve_grad = get_grad_from_net(net)
        ...
        if i % print_interval == (print_interval-1):    # print every few mini-batches
            print(f'[{epoch + 1}, {i + 1:5d}] loss: {running_loss * 1.0 / print_interval:.3f}')
