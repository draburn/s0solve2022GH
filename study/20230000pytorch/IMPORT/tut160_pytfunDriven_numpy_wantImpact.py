...
def get_assigned_data_to_x__old(this_data, this_x):
    sz = this_data.size()
    num_dim = len(sz)
    nx = 0
    if (1==num_dim):
...
def get_assigned_data_to_x(this_data, this_x):
    this_x = numpy.reshape(this_data.numpy(),-1)
    return this_x

def get_assigned_net_to_x__old(this_net, this_x):
...
def get_assigned_net_to_grad___old(this_net, this_grad):
    index_list = get_index_list(this_net)
...
def assign_x_to_data(x, d):
    d = torch.from_numpy( numpy.asarray(x) )
...
def assign_x_to_net__doesnotworkforsomereason(this_x, this_net):
    index_list = get_index_list(this_net)
    this_net.conv1.bias.data   = torch.from_numpy(numpy.asarray(this_x[index_list[0]:index_list[1]]))
    this_net.conv1.weight.data = torch.from_numpy(numpy.asarray(this_x[index_list[1]:index_list[2]]))
    this_net.conv2.bias.data   = torch.from_numpy(numpy.asarray(this_x[index_list[2]:index_list[3]]))
    this_net.conv2.weight.data = torch.from_numpy(numpy.asarray(this_x[index_list[3]:index_list[4]]))
    this_net.fc1.bias.data     = torch.from_numpy(numpy.asarray(this_x[index_list[4]:index_list[5]]))
    this_net.fc1.weight.data   = torch.from_numpy(numpy.asarray(this_x[index_list[5]:index_list[6]]))
    this_net.fc2.bias.data     = torch.from_numpy(numpy.asarray(this_x[index_list[6]:index_list[7]]))
    this_net.fc2.weight.data   = torch.from_numpy(numpy.asarray(this_x[index_list[7]:index_list[8]]))
    this_net.fc3.bias.data     = torch.from_numpy(numpy.asarray(this_x[index_list[8]:index_list[9]]))
    this_net.fc3.weight.data   = torch.from_numpy(numpy.asarray(this_x[index_list[9]:index_list[10]]))
    this_net.conv1.bias.data[0] = 999.0
    this_net.fc3.weight.data[-1][-1] = 666.0

def get_dat_from_x(this_x, this_index0, this_index1, this_dat ):
    print(f"this_dat.size() = {this_dat.size()}")
    #return torch.from_numpy( numpy.reshape( numpy.asarray(this_x[this_index0:this_index1]), this_dat.size() ) )
    foo = torch.from_numpy( numpy.reshape( numpy.asarray(this_x[this_index0:this_index1]), this_dat.size() ) )
    print(f"foo.size() = {foo.size()}")
    return foo
    #return torch.from_numpy( numpy.reshape( numpy.asarray(this_x[this_index0:this_index1]), this_dat.size() ) )
...
for epoch in range(2):  # loop over the dataset multiple times
    ...
    for i, data in enumerate(trainloader, 0):
        ...
        if (test_and_quit):
            ...
            print("WE NEED SOME SORT OF 'SIZE' ARGUMENT FOR numpy.asarray()!")
            ###net.conv1.bias.data = torch.from_numpy(numpy.asarray(sxsolve_x[index_list[0]:index_list[1]]))
            net.conv1.bias.data = get_dat_from_x(sxsolve_x, index_list[0], index_list[1], net.conv1.bias.data )
            #net.fc1.weight.data[0][0] = 3.14159
            ###net.fc1.weight.data = torch.from_numpy(numpy.asarray(sxsolve_x[index_list[5]:index_list[6]]))
            ...
            exit()
            #
            net.conv1.weight.data = torch.from_numpy(numpy.asarray(sxsolve_x[index_list[1]:index_list[2]]))
            net.conv2.bias.data   = torch.from_numpy(numpy.asarray(sxsolve_x[index_list[2]:index_list[3]]))
            net.conv2.weight.data = torch.from_numpy(numpy.asarray(sxsolve_x[index_list[3]:index_list[4]]))
            net.fc1.bias.data     = torch.from_numpy(numpy.asarray(sxsolve_x[index_list[4]:index_list[5]]))
            net.fc1.weight.data   = torch.from_numpy(numpy.asarray(sxsolve_x[index_list[5]:index_list[6]]))
            net.fc2.bias.data     = torch.from_numpy(numpy.asarray(sxsolve_x[index_list[6]:index_list[7]]))
            net.fc2.weight.data   = torch.from_numpy(numpy.asarray(sxsolve_x[index_list[7]:index_list[8]]))
            net.fc3.bias.data     = torch.from_numpy(numpy.asarray(sxsolve_x[index_list[8]:index_list[9]]))
            net.fc3.weight.data   = torch.from_numpy(numpy.asarray(sxsolve_x[index_list[9]:index_list[10]]))
            print("After 'assign_x_to_net'...")
            print(f"net.conv1.bias.data[0] = {net.conv1.bias.data[0]:.17e}")
            print(f"net.fc1.weight.data[0][0] = {net.fc3.weight.data[0][0]:.17e}")
            print(f"net.fc3.weight.data[-1][-1] = {net.fc3.weight.data[-1][-1]:.17e}")
            ...
            #print(net.conv1.bias.data.array())
            #print(net.conv1.bias.data.numpy())
            #
            #print(net.conv1.weight.data)
            #print(net.conv1.weight.data.array())
            #print(net.conv1.weight.data.numpy())
        else:
