...
cx = 0.01;
c0 = (c1^2)*(1.0+cx)/4.0;
funchF = @(x) blm0516_func( x, matA, matB, vecX0, indexNNonLin, c1, c0 );
funchFNorm = @(x)sqrt(sum(funchF(x).^2,1));
%
figIndex++; figure(figIndex);
funchVizXY = @(x,y)asinh(funchFNorm([x;y])*100)/100;
contourfunch( funchVizXY, [0.15,1.3,0.99,1.04] );
%contourfunch( funchVizXY, [0.2076,0.2077,1.002394,1.002400] );
cMap = 0.6+0.4*jet;
cMap(1,:) *= 0.4;
colormap(cMap);
grid on;
...
%
numVals = 1001;
rvecLambda = linspace(-1E-6,1E-6,numVals);
