	...
	%
	%
	%options_fsolve = optimset( "Display", "iter", 'Jacobian', 'off', 'GradObj', 'off' );
	for n = 1 : 16
	...
	%
	semilogy( ...
	  fevalCountVals_fsolve, omegaVals_fsolve, 'o-', ...
	  datOut_rpool_scan.fevalCountVals, sumsq(datOut_rpool_scan.vecFVals,1)/2.0, 'x-' );
	grid on;
	return;
	%
	msg( __FILE__, __LINE__, "" );
