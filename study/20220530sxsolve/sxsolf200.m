% New to sxsolf200 (planned)...
%   -> reconsider whether to pass x, y(explored space), y(locale space, "z"?);
%   -> pay a bit more attention to optimization; at least share chol() for (full-Newton) unbound and bound explored space;
%   -> keep "archive" of { x, f, V, W, epsUsed } at each "locale", in addition to "VExplored" and "WExplored";
%       this could be passed to an arbitrary preconditioner, later;
%   -> structure to allow arbitrary (full-space) preconditioners, including sensed diagonal and sense sparse matrices;
%   -> decide between using a (full-space) preconditioner and our (history/observation/explored) {V,W};
%      ! in this vein, also consider taking a step per each of those and per local {V,W}.
%   -> allow "expand subspace" that's not perfectly orthogonal to existing explored space;
%      ( not yet totally sure how if/when to use it).
%   -> commit to main loop framework in s100, where main loop merely keeps track of best and
%      fModelDat is the one that considers "locales" and "steps".
%    ~ reconsider structure for hackish stuff:
%       coasting (blind steps in explored spcae without calculating local {V,W});
%       temporarily rejected steps;
%   -> allow for "gliding": blindly relying on full-space precon;
%   -> require regularization of matB, the "boundary" matrix, at least after the first trial step.

function [ vecX, vecF, retCode, fevalCount, stepsCount, datOut ] = sxsolf200( funchF, vecX_initial, vecF_initial=[], prmIn=[] )
