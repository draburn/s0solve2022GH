function x = calcLinishRootOfQuad( a, b, c, tol=sqrt(eps) )
	thisFile = "calcLinishRootOfQuad";
	% y = (a*(x^2)) + (b*x) + c;
	% This calculates the root that logically corresponds to the root
	%  of the quadratic when the quadratic term is zero.
	% I previously called this the "good" root, but, now feel that naming
	%  is misleading.
...
%!	xGP = calcLinishRootOfQuad( a, b, c )
%!	yGP = funchY(xGP)
